...
B19 remains in progress after classification. Its next sub-batches must turn
`SupportedUnselected` declarations into emitted bindings and replace broad LT001-LT004
blockers with implemented projection/ownership rules or narrow accepted reasons.

B20 release engineering is implemented but the B20 batch remains open. The machine-readable
...
Batch progress is 19/21 (B00-B18 complete) until B19 and B20 satisfy their original
completion gates. Classification percentage, emitted binding coverage, engineering
implementation, and publication readiness remain independent metrics.
...
- Local release scripts, SBOM, provenance, and CI configuration remain valid B20 work and
  do not need to be discarded.
- New B19 work uses coherent package/ownership sub-batches, normally 100-500 low-risk
  declarations or a smaller high-risk lifetime family, with generated-source, build,
  runtime, package, and documentation evidence.
...
- Every B19 sub-batch must increase emitted/manual coverage or resolve a real blocker; a
