...
ABI 1.58, bridge 0.66.0, and schema 1.13. Sketch workflows belong to the OcctSharp
facade; existing public types and module owners are unchanged. Definitions copy their
inputs and edits return new values; generated bindings are regenerated by the normal
pipeline. Interpolate is explicitly piecewise linear. Bounds are conservative and
include topology tolerance. STEP preserves named layers; IGES may assign numeric
layers to transferred sublabels. Final API comparison is recorded in STATUS.
