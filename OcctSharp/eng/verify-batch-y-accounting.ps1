...
if ((Get-FileHash -LiteralPath $beforeFile -Algorithm SHA256).Hash -cne
    ...
    throw 'Batch Y entry inventory is not the accepted Preview.23 baseline.'
}
...
if ($oldIds.Count -ne 19682 -or -not $oldIds.IsSubsetOf($currentIds)) { throw 'Baseline generated IDs were lost or changed.' }
$added = [Collections.Generic.HashSet[string]]::new($currentIds, [StringComparer]::Ordinal)
$added.ExceptWith($oldIds)
if ($added.Count -ne 968) { throw 'The accepted Y stable-ID delta changed.' }
...
$refinementFile = Join-Path $workspace 'config/batches/batch-y-disposition-refinements.json'
...
foreach ($old in $before.FinalClassification.Declarations) {
    ...
    elseif ($id -cin @(
        'c:@S@VrmlData_IndexedFaceSet@F@GetNormal#I#I#',
        'c:@S@Select3D_SensitiveCircle@F@Select3D_SensitiveCircle#&1$@N@opencascade@S@handle>#$@S@SelectMgr_EntityOwner#&1$@S@gp_Circ#b#I#')) {
        if ($old.State -ne 'Blocked' -or $new.State -ne 'Skipped' -or $new.Code -ne 'SK008' -or $new.Category -ne 'ArtifactUnavailable') {
            throw 'An exact Y artifact exception changed.'
        }
    }
...
if ($refinements.Count -ne 223 -or $seenRefinements -ne $refinements.Count) { throw 'Refinement audit is incomplete.' }
...
Write-Host "Batch Y accounting PASS: $($added.Count) Blocked-to-Emitted IDs; two exact exceptions; $seenRefinements audited Blocked reason refinements; no other disposition or header changes."
