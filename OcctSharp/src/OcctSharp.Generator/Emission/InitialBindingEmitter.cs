...
public static class InitialBindingEmitter
{
        ...
        IReadOnlyList<string> generatedPreambleHeaders)
    {
            ...
            .OrderBy(static pair => pair.Key))
        {
            ...
            if (moduleMethods.Length > 0)
            {
                files.Add(new GeneratedFile(
                    ...
                    UseModuleNativeMethods(EmitStaticManagedBindings(moduleMethods), module),
                    module,
                    GeneratedApiLayer.Raw,
                    "Values.ManagedRaw"));
            }
        ...
        foreach (OcctProductModule module in files
            .Where(static file => file.RelativePath.EndsWith(".cs", StringComparison.Ordinal))
            .Select(static file => file.ProductModule)
            .Distinct()
            .Order())
        {
            string moduleName = module.ToString();
            files.Add(new GeneratedFile(
                $"src/OcctSharp/Generated/{moduleName}/{moduleName}.ModuleRuntime.Generated.cs",
                EmitManagedModuleRuntime(module),
                module,
                GeneratedApiLayer.Runtime,
                "Managed.ModuleRuntime"));
        }
    ...
    private static string EmitManagedModuleRuntime(OcctProductModule module) =>
        $$"""
        // <auto-generated />

        namespace OcctSharp.Generated;

        internal static partial class {{module}}GeneratedNativeMethods
        {
            private const string LibraryName = "OcctSharp.Native";

            static {{module}}GeneratedNativeMethods() =>
                global::OcctSharp.Interop.NativeLibraryResolver.EnsureRegistered(typeof({{module}}GeneratedNativeMethods).Assembly);
        }
        """ + Environment.NewLine;
